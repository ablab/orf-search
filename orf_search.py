...
def main(args):
    ...
    if len(args) == 2 and args[1] == "--test":
        ...
        args = [args[0], "-m", PKG_LOCATION  + "/tiny_dataset/ricinb_lectin2.hmm", "-s", PKG_LOCATION  + "/tiny_dataset/toxin.fasta", \
                         "-r", "-g", PKG_LOCATION  + "/tiny_dataset/graph.gfa", "-k", "55", "-o", test_dir]
    ...
    if hmm_return_code == 0 and sequences_filename != None and not os.path.exists(join(args.out, hmms_name + ".dtbl")):
        find_true_hmm_alignments(sequences_filename, args.hmms, str(evalue), t, join(args.out, hmms_name))
